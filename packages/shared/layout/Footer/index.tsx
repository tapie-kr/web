...
export default function Footer() {
  return <>
    <Flex tag='footer' className={footer} justify='between' fullWidth gap={spacingVars.large}>
      <VStack gap={spacingVars.moderate}>
        <VStack fitContent align='start' gap={spacingVars.petite}>
          <TAPIESymbol size={TAPIESymbolSize._20} hasLabel />
          <Typo.Base color={colorVars.content.muted} weight={Weight.MEDIUM}>
            서울특별시 용산구 원효로97길 33-4 <br />
            선린인터넷고등학교 343실
          </Typo.Base>
        </VStack>
        <Typo.Petite color={colorVars.content.muted}>ⓒ 2025 TAPIE. All rights reserved.</Typo.Petite>
      </VStack>

      <HStack className={linkList} wrap gap={spacingVars.base} align='end'>
        <LinkChip
...
function LinkChip(props: LinkChipProps) {
  return <>
    <Link href={props.href} target='_blank'>
      <HStack className={linkChip} align='center' gap={spacingVars.tiny}>
